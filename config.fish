if set -q ITERM_PROFILE
    ...
    set PATH $PATH /usr/local/Cellar
    set PATH $PATH /usr/local/share/npm/bin
    set PATH $PATH $HOME/bin
    #set PATH $PATH /usr/lib/jvm/java-6-openjdk-amd64/bin
    ...
    alias ejh-keepass="/Applications/KeePassX.app/Contents/MacOS/KeePassX ~/Dropbox/keys/ehazlett.kdb"
    alias tmux "tmux -2"
    alias vimlight="sed -i 's/background=dark/background=light/g' ~/.vimrc"
    alias vimdark="sed -i 's/background=light/background=dark/g' ~/.vimrc"
...
alias sshhome='mosh --ssh="ssh -i ~/Dropbox/keys/home_ehazlett -o StrictHostKeyChecking=no " '
alias ssharcus='ssh -i ~/Dropbox/keys/arcus_ehazlett -o StrictHostKeyChecking=no '
alias scparcus='scp -i ~/Dropbox/keys/arcus_ehazlett '
alias sshamara='ssh -i ~/Dropbox/keys/ehazlett_pculture '
...
function d
    if math "1<" (count $argv) > /dev/null
        docker run -h $argv[1] -it --restart=always -v ~/Sync:/home/dev/Sync --name $argv[1] -v /var/run/docker.sock:/var/run/docker.sock $argv[2..(count $argv)] ehazlett/devbox fish
    else
        docker run -h $argv[1] -it --restart=always -v ~/Sync:/home/dev/Sync --name $argv[1] -v /var/run/docker.sock:/var/run/docker.sock ehazlett/devbox fish
    end
end
